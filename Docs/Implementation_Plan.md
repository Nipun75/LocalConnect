...
2. **P1 — Differentiation and polish**
   ...
   - Post a Need (reuses the Phase 4/5/8 pipeline — see Phase 8A; not a separate system)
   - Match explanations
   - PWA installability
...
## MVP Formula — CANONICAL, matches TRD §8.4 and Backend Schema §22 exactly
...
Experience and response rate influence trust/ranking data rather than create too many independent factors — there is no separate `experience_score` column (TRD §8.11).

Two rules that exist nowhere else and must be implemented exactly as written:
- **Rating dampening:** if a provider has fewer than 3 reviews, use `rating_score = 0.5` instead of the raw `average_rating / 5`.
- **Unknown price:** if provider pricing is unknown, use `price_score = 0.3` — not 0 (which implies confirmed unaffordable) and not 1.0 (which implies confirmed affordable).
...
## Minimum Match Threshold — CANONICAL VALUE (matches TRD §8.13)

**40/100.** Chosen over a stricter 50/100 floor deliberately: Trust + Rating together are only 20% of the formula, so a 50-floor would filter out perfectly good new providers before they've built history. This is provisional pending real usage data.
...
## MVP Trust Model — CANONICAL, matches TRD §9.2-9.3 and Backend Schema §15 exactly

Score range: **0–100**.

```text
Phone Verified            15%
Identity Submitted        10%
...
Two changes from earlier drafts of this phase: **"Identity verification" is renamed "Identity Submitted"** (10%, down from 15%) and **Profile Completeness moves from 10% to 15%**. There is no real identity-verification mechanism in MVP — only document submission — so the label must say "Submitted," never "Verified," and its weight should sit below Phone Verified (which does have a real check behind it). The freed 5% goes to Profile Completeness, which previously had the lowest weight of any real, implementable signal.

Per-signal computation (all defined precisely in TRD §9.2 — do not re-derive these while coding, use the table there):
- Phone Verified / Identity Submitted: binary, from `phone_verified_at` / `identity_submitted_at`
- Profile Completeness: 7-field checklist, see TRD §9.2
- Rating: `average_rating/5`, but use `0.5` if `total_reviews < 3`
- Completed Jobs: `LEAST(100, completed_jobs × 5)`
- Recommendations: `LEAST(100, recommendation_count × 10)`
- Response Rate: computed by the `refresh_response_rate()` trigger (Backend Schema §21) — this was previously undefined everywhere; it now has a real source

| ID | Task | Owner | Dependencies | Priority | Effort | Definition of Done |
|---|---|---|---|---|---|---|
| P6-001 | Implement trust input queries | D2 | P3 | P0 | 1.5h | Trust signals available |
| P6-002 | Implement normalized trust calculation | D3 | P6-001 | P0 | 2h | Score is deterministic, matches canonical formula exactly |
| P6-003 | Store/recalculate trust | D2+D3 | P6-002 | P0 | 1.5h | Score can be recalculated |
| P6-004 | Trust badge UI (4 badges) | D1 | P6-003 | P0 | 1.5h | Phone Verified / Identity Submitted / Profile Verified / Community Recommended each render from real data, none hardcoded |
| P6-005 | Trust explanation | D3 | P6-002 | P0 | 1h | User understands score |
| P6-006 | Trust tests | D3 | P6-002 | P0 | 1h | Boundary cases pass, including <3-review dampening and 0-signal cold start |
| P6-007 | Response rate trigger | D2 | P3 | P0 | 1h | `response_rate` updates when `request_matches.provider_response` is set — no other document defined this before |
| P6-008 | Phone verification flow | D1+D2 | P2 | P0 | 2h | `phone_verified_at` set via a real (even if minimal) OTP/SMS check |
| P6-009 | Identity submission flow | D1+D2 | P2 | P1 | 1.5h | `identity_submitted_at` set on document upload; badge text says "Submitted" |

Trust should be recalculated whenever material signals change (see TRD §9.5 for the full event list), or on demand for the MVP.
...
This targets `provider_recommendations` (Backend Schema §16a), **not** the existing `recommendations` table — that table models an unrelated AI-suggestions concept and is left unused for MVP. A recommendation **requires** a completed connection between recommender and provider; this is a hard requirement, not optional, since an optional check can't actually be enforced (PRD §21).

| ID | Task | Owner | Dependencies | Priority | Effort | Definition of Done |
|---|---|---|---|---|---|---|
| P7-001 | `provider_recommendations` migration + eligibility trigger | D2 | P3, Phase 8 connections | P1 | 1.5h | Table, unique constraint, and eligibility trigger (completed connection required, self-recommendation blocked) exist |
| P7-002 | Recommendation creation UI | D1 | P7-001 | P1 | 1.5h | Authorized user can recommend a provider they had a completed connection with |
| P7-003 | Recommendation display | D1 | P7-001 | P1 | 1.5h | Provider profile shows "Recommended by N nearby" from a real count |
| P7-004 | Recommendation count → trust | D2+D3 | P7-001 | P1 | 0.5h | `recommendation_count` feeds Trust Score's Recommendations factor (10%) |
| P7-005 | Revoke recommendation | D1+D2 | P7-001 | P2 | 1h | Recommender can deactivate their own recommendation |
| P7-006 | Seed recommendations | D2 | P7-001, seeded completed connections | P1 | 0.5h | Demo providers have social proof; seeded via `service_role` against seeded `completed` connections |
...
**Note on P8-001:** AI parsing and matching (Phases 4–5) run statelessly and do not require a `requests` row to exist — see TRD §14.4 and §11.3. The persisted `requests` row this phase creates is written at the moment of commitment (Connect, or Post a Need), which is also the point authentication becomes required. A guest can see fully parsed, ranked, explained results before this task's service is ever called.

| ID | Task | Owner | Dependencies | Priority | Effort | Definition of Done |
|---|---|---|---|---|---|---|
| P8-001 | Request creation service (persists at commit time — see note above) | D2 | Auth, provider data | P0 | 2h | Authenticated user can persist a request at Connect/Post-a-Need time |
...
| P8-008 | Dual-confirmation completion | D2 | P8-006 | P0 | 1.5h | `confirm_connection_completion()` wired up; neither party can complete unilaterally (Backend Schema §20a) |
| P8-009 | "Mark complete" UI, both sides | D1 | P8-008 | P0 | 1.5h | Requester and provider each see a "Mark as completed" action; UI reflects "waiting on the other party" between the two confirmations |

### Request/Connection Statuses

Use a small state machine, now with an explicit completion gate:
...
connected
  ↓
(requester confirms AND provider confirms — either order)
  ↓
completed
```
...
`completed` is reachable **only** once both participants have independently confirmed — see Backend Schema §20a / TRD §9.6. This is the fix for the critical issue where either party could previously flip a connection to completed alone, inflating Trust Score with no real service having occurred.

Avoid a large workflow for the hackathon beyond this.

---

# Phase 8A — Post a Need (P1, reuses Phase 4/5/8)

**Goal:** Ship FR-15 without building a second product system. Previously this feature had zero tasks anywhere in this plan despite being listed as Must-Have in the PRD — it is now explicitly scoped and P1 (PRD §14/§22).

This phase adds **no new tables**. It reuses the same `requests` row, the same AI parsing (Phase 4), and the same `request_matches` generation (Phase 5) as the standard search flow. The only new work is presentation: a "publish and wait" screen instead of an immediate results screen, and a provider-facing view of pending matches.

| ID | Task | Owner | Dependencies | Priority | Effort | Definition of Done |
|---|---|---|---|---|---|---|
| P8A-001 | "Need published" confirmation screen | D1 | P8-001 | P1 | 1.5h | User sees "your need is live" instead of immediate results |
| P8A-002 | "Nearby Open Needs" provider view | D1 | P5-009 | P1 | 2h | Provider sees `request_matches WHERE status = 'pending'` assigned to them, in list form |
| P8A-003 | Provider interest action | D2 | P8-004 | P1 | 0.5h | Reuses the existing accept/decline action from P8-004 — no new endpoint |
| P8A-004 | Customer comparison view | D1 | P5-010 | P1 | 1h | Reuses the existing results-list component (P5-010), fed from provider-initiated matches instead of AI-ranked ones |

If Milestone F/G time is tight, this phase is the first thing to cut — it is explicitly P1, not P0, and the core NEED→MATCH→CONNECT demo does not depend on it.
...
- **Price unknown → 0.3, not 0 and not 1.0** (TRD §8.7) — don't assume unknown pricing fits, but don't treat it as confirmed too expensive either
...
Trust is derived from actual stored signals — see Phase 6's canonical formula, which this section must match:
...
| Fake reviews | High | Medium | Reviews gated to completed connections only, one per connection (Backend Schema §14) |
| Self-inflated trust/match scores | High | Low | Column-level GRANT/REVOKE + dual-confirmation completion gate (Backend Schema §20a) — closes the RLS gap where either party could previously flip a connection to `completed` alone |
