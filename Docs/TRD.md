...
## 8.4 Score — CANONICAL MATCH SCORE FORMULA

This is the single source of truth for the Match Score. Backend Schema §22 and Implementation Plan Phase 5 must match this exactly.
...
**Experience is not an independent scoring factor.** Earlier drafts of this document scored experience separately (5%); it has been folded out. Experience years are still displayed on the provider card as context, and influence Trust indirectly through Completed Jobs, but the `request_matches` table does not — and will not — store a separate `experience_score` column. This keeps the persisted schema and the formula in agreement (see Backend Schema §12/§21).
...
If provider pricing is unknown, do not assume it fits. Score unknown pricing at **0.3** — the same treatment as unknown availability (§8.8) — rather than 0 (which would equate "unknown" with "confirmed too expensive") or 1.0 (which would equate "unknown" with "confirmed affordable").
...
**Minimum review-count safeguard:** if `total_reviews < 3`, use `rating_score = 0.5` (neutral) instead of the raw average. This prevents a single early five-star (or one-star) review from dominating the score before there is enough evidence to trust it. Once `total_reviews >= 3`, use the true average.

## 8.11 Experience — Not an Independent Score

Experience is no longer a fifth scoring factor (see §8.4). `experience_years` remains a stored, displayed attribute on the provider card, and contributes to Trust indirectly via Completed Jobs — it is not separately weighted in the Match Score.
...
## 8.13 Minimum Threshold — CANONICAL VALUE

```text
Match Score >= 0.40  (40/100)
```

This is deliberately lower than a stricter 0.50 floor would be. With Trust + Rating together worth only 20% of the formula, a 0.50 floor risks filtering out perfectly good new providers before they've had a chance to accumulate history — directly undermining the cold-start principle in §9.4 and the PRD's "Individuals Matter" product principle (PRD §13.5). This number is provisional pending real usage data; revisit after the first demo/pilot run.

The threshold should remain configurable, not hard-coded.
...
## 9.2 Signals — CANONICAL TRUST SCORE FORMULA

This is the single source of truth for the Trust Score. Backend Schema §15 (including the `recalculate_provider_trust()` function) and Implementation Plan Phase 6 must match this exactly. The PRD's example (§20) illustrates it but this section governs the number.

| Signal | Weight | Normalization | Missing-data behavior |
|---|---:|---|---|
| Phone Verified | 15% | Binary: `phone_verified_at IS NOT NULL` → 100, else 0 | Defaults to 0 (unverified) |
| Identity Submitted | 10% | Binary: `identity_submitted_at IS NOT NULL` → 100, else 0. **This is submission, not verification** — never label the badge "Identity Verified." | Defaults to 0 |
| Profile Completeness | 15% | `(fields_filled / 7) × 100`, where the 7 fields are: `display_name`, `bio`, ≥1 `provider_services` row, ≥1 `provider_availability` row, `service_radius_km` set, a primary `locations` row, `avatar_url` | Unfilled fields simply don't count — no partial credit assumed |
| Rating | 20% | `(average_rating / 5) × 100`; if `total_reviews < 3`, use 50 (neutral) instead of the raw value — same safeguard as Match Score §8.10 | No reviews → `average_rating` defaults to 0, rating factor = 0 (not 50, since 0 reviews ≠ "a few unreliable reviews") |
| Completed Jobs | 15% | `LEAST(100, completed_jobs × 5)` — 20 completed jobs reaches the cap | 0 jobs → 0 |
| Community Recommendations | 10% | `LEAST(100, recommendation_count × 10)` — 10 active recommendations reaches the cap | 0 recommendations → 0 |
| Response Rate | 15% | `(responded_matches / total_matches_sent) × 100`, all-time, computed from `request_matches` (see Backend Schema §15.1) | `total_matches_sent = 0` → 0, not undefined and not 100 |

Total: **100%**

Weights are lower for Identity Submitted (10%) than Phone Verified (15%) deliberately — identity submission has no verification step behind it for MVP, so it should carry less weight than a signal with an actual (even if lightweight) check.
...
Trust Score =
  0.15 × Phone Verified
+ 0.10 × Identity Submitted
+ 0.15 × Profile Completeness
+ 0.20 × Rating
+ 0.15 × Completed Jobs
+ 0.10 × Community Recommendations
+ 0.15 × Response Rate
```

Rounded to an integer, range **0–100**.

## 9.4 Cold-Start Handling — EXACT THRESHOLD

A new provider should not receive an artificially high score because of missing history.

**Threshold (previously undefined anywhere — this closes that gap):**

```text
completed_jobs = 0 AND total_reviews = 0  →  show "New provider" badge
```

The Trust Score itself is not artificially boosted or suppressed — it naturally comes out lower because two of the seven canonical factors (Completed Jobs, Rating) are legitimately 0. The badge exists to explain *why* the number is what it is, not to change the number.

Example:

```text
New provider:
phone verified + profile complete
but 0 jobs, 0 reviews
```
...
- profile update (any of the 7 completeness fields)
- review submitted or updated
- connection status transitions to `completed` (see §9.6 completion gate — this must be a verified transition, not a unilateral one)
- recommendation submitted or revoked
- provider response recorded on a `request_matches` row (feeds Response Rate — see §9.7)
...
`completed_jobs` and its downstream Trust Score contribution must never be settable by a single unilateral action. A connection may transition to `completed` only once **both** the requester and the provider have independently confirmed it — neither party can trigger the transition alone. See Backend Schema §13a for the enforced mechanism (`requester_confirmed_at` / `provider_confirmed_at`, gated trigger). This closes the gap where either party could flip a connection to `completed` with no service having occurred, inflating Trust Score with no real signal behind it.

## 9.7 Response Rate Computation

No prior version of this document specified how Response Rate is actually computed; it existed only as a formula weight with no source. It is defined as:

```text
response_rate =
  (request_matches where provider_response IS NOT NULL)
  / (request_matches assigned to this provider, all-time)
  × 100
```

Recalculated whenever a `request_matches.provider_response` value is set (i.e., the provider accepts, declines, or otherwise responds). A provider with zero assigned matches has `response_rate = 0`, not `NULL` and not 100 — no response history is not evidence of good response behavior.
...
Public — resolved, no longer conditional:
...
Unauthenticated (guest) users can browse all of the above, and can also submit a natural-language need and see AI-parsed, ranked match results — see §14.4's statelessness note. What requires authentication is committing to a specific provider (Connect) or persisting a need (Post a Need), not viewing results. This was previously left as "depending on the product's final privacy policy"; that policy is now decided.
...
A user vouching for a provider they had a completed connection with. Modeled as `provider_recommendations` (Backend Schema §16) — distinct from the schema's separate `recommendations` table, which models an unrelated AI-suggestion concept and is not used by this feature. Do not conflate the two.
...
**Statelessness and guest access:** note that this endpoint's input is `structured_requirement` (a value), not a `request_id`. Matching does not require a persisted `requests` row. This is what makes guest browsing possible without any change to `requests.requester_id NOT NULL` (Backend Schema §21): an unauthenticated user can have their need parsed and matched entirely in-memory, see ranked results with explanations, and only cause a `requests` row to be written at the moment they authenticate and either click Connect or explicitly choose Post a Need. See §11.3.
