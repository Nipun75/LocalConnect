...
The PRD explicitly positions the product around this loop and states that everything else should support the core **Need → Match → Connect** experience.
...
The TRD defines minimum routes around authentication, search, matches, provider profiles, requests, customer profile, and provider profile/requests.
...
Location is optional; the TRD explicitly requires the product to remain usable when browser location permission is denied.
...
**Resolved (Decision Log #5):** matching runs statelessly — see TRD §14.4 and §11.3. A guest can describe a need, see AI-parsed requirement chips, and see ranked, explained match results with zero account. No `requests` row is written until the user commits. Authentication is required only at the point of commitment: **Connect**, or explicitly choosing **Post a Need**. The sequence below was already correct; this section previously described it without the schema-level explanation of why it works despite `requests.requester_id` being `NOT NULL`.
...
Authentication is required for service requests, profile editing, reviews, and recommendations according to the TRD.
...
The PRD explicitly supports natural-language English/Hinglish input and editable extracted requirements.
...
The TRD requires AI output to be schema-validated and business-validated before matching.
...
The AI must not invent missing requirements. Unknown values remain unknown/null rather than being silently fabricated.
...
This deterministic matching pipeline is specified in the TRD.
...
The PRD requires match explanations based on actual ranking signals rather than opaque AI decisions.
...
These are defined as provider-profile information in the PRD.
...
Do not turn the MVP into an analytics-heavy provider dashboard. The PRD explicitly excludes complex analytics from MVP scope.
...
These categories are defined as MVP examples in the product material.
...
For example, unknown availability is not automatically treated as available. The TRD explicitly distinguishes unknown from available in availability scoring.

---

# H. POST A NEED FLOW

**Priority: Should Have (P1)** — downgraded from Must Have (PRD §14/§22, Decision Log #4). This flow reuses the same `requests` → AI parsing → `request_matches` pipeline as the standard search flow (sections C–E, I); it is a presentation variant, not a separate broadcast/bidding system. "Relevant Providers" (step 8 below) is the same `request_matches` data a normal search generates, just filtered to providers browsing open needs instead of the AI ranking them. See Implementation Plan Phase 8A.
...
Invalid budgets are rejected; missing budgets are not invented.
...
The PRD explicitly describes Post a Need as a simple request-response experience rather than a complex bidding marketplace.
...
The TRD explicitly defines candidate retrieval before scoring and requires server-side filtering rather than fetching all providers into the browser.

## I.3 Ranking

The canonical MVP weighting (TRD §8.4, Backend Schema §22, Implementation Plan Phase 5 — all three now match exactly):
...
There is no separate Experience factor — experience is displayed as context on the provider card but does not independently affect ranking (TRD §8.11).

This is a technical matching rule, while the UX responsibility is to expose the resulting reasons transparently.

## I.4 Match Threshold

**40/100** (TRD §8.13, Implementation Plan Phase 5 — canonical). If no provider reaches this:

```text
No strong matches found
```

Do not present poor matches as strong matches.
...
The PRD defines Trust Score as an evidence-based indicator and explicitly says it should not be presented as a guarantee that someone is definitely trustworthy. All 7 branches shown here correspond exactly to the Canonical Trust Score Formula's 7 weighted factors (TRD §9.2) — nothing is displayed here that isn't actually an input to the number.
...
Do not present any of these as government/KYC verification unless such verification actually exists.
...
User chooses Recommend Provider
 ↓
Eligibility check: connection is completed, recommender was a participant, not self-recommendation
 ↓
...
**Recommendations require a completed connection — this is enforced, not a soft guideline.** An "optional connection reference" (an earlier draft's language) cannot actually be checked; requiring it is what makes "genuine interaction" mean something (Backend Schema §16a). A user cannot recommend the same provider twice (unique constraint) and cannot recommend their own provider profile. The PRD distinguishes recommendations from ordinary reviews: recommendations act as a community trust signal.
...
The TRD requires availability to influence matching and says provider availability should be rechecked before the final connection action where applicable.
...
The TRD explicitly requires manual locality/address fallback and says exact customer coordinates should not be exposed by default.
...
**COMPLETED is reached only once both participants have independently confirmed** — see Backend Schema §20a / TRD §9.6. Neither the customer nor the provider can mark a connection complete alone; this closes a critical gap where either side could previously self-report completion and inflate Trust Score with no real service having occurred. See M.4 for the UI implication.

The TRD recommends this small lifecycle for MVP; this diagram is the presentation-layer composition of the three underlying database enums (`request_status`, `match_status`, `connection_status` — Backend Schema §11/§21). This app-facing flow does not need to expose all three separately to the user.
...
Either the customer or provider has confirmed the service is done; waiting on the other party. The UI should show which side is still pending — e.g., "You marked this complete. Waiting for [provider] to confirm."

### Completed

Both customer and provider confirmed. Interaction/service is complete.
...
The provider profile requirements include skills, categories, experience, pricing, availability, service area, description, and contact preference.
...
The system must not invent providers.
...
The product must remain usable without GPS.
...
The fallback sequence is directly supported by the TRD.
...
The TRD explicitly prohibits inventing missing requirements.
...
Continue without budget matching rather than inventing a value.
...
The provider should not normally rank as available when marked unavailable.
...
The TRD explicitly requires these states to remain distinct.
...
This behavior is explicitly defined in the TRD.
...
The TRD requires an offline fallback and explicitly says full offline-first request creation is not required for MVP.
...
The PRD defines a 2–3 minute demo around natural-language input → AI extraction → nearby matches → Trust Score → community recommendations → provider profile → connection → provider receiving the request.
...
The PRD explicitly keeps payments, complex KYC, advanced bidding, full social networking, complex analytics, and advanced fraud detection outside the MVP.
