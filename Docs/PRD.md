...
- Post a Need *(P1 — see §22; reuses the NEED→MATCH pipeline rather than a separate broadcast system)*
- Map discovery
- Saved providers
...
**Identity Submitted** means a document was uploaded, not that it was verified. Do not label it "Identity Verified."

> The exact weights, normalization, and computation for each signal are locked in the **Canonical Trust Score Formula** (TRD §9, Backend Schema §15). This section describes product intent; the TRD/Schema are the source of truth for the number itself.
...
A recommendation **must** be linked to a completed connection between the recommender and the provider. This is a hard requirement, not a soft preference — it is the only mechanism that makes "genuinely interacted with" enforceable rather than aspirational.

A user cannot recommend the same provider more than once (one active recommendation per user/provider relationship, enforced by a unique constraint).

A user cannot recommend a provider profile that is their own account.

Implemented safeguards (see Backend Schema §16 for the `provider_recommendations` table):

- One active recommendation per user/provider relationship (unique constraint)
- Required link to a completed connection (no connection, no recommendation)
- Self-recommendation blocked
- Recommender can revoke (deactivate) their own recommendation
- Public recommendation count is cached and derived only from active recommendations

This is a deliberately narrower rule than early drafts of this document suggested ("optional connection reference"). The connection is required for MVP because an optional check cannot actually be enforced or verified.
...
**Priority: Should Have (P1).** Downgraded from Must Have — see §14. Post a Need is not one of the four named MVP differentiators (§1); it is a presentation variant of the core NEED → MATCH loop, not a second product system.

Users can publish a requirement when they do not want to browse ranked results immediately.
...
1. User creates need (same `requests` row and AI parsing as the standard search flow — no separate data model).
2. AI structures the requirement.
3. User confirms.
4. Instead of seeing ranked results immediately, the user sees "Your need is live — providers will respond."
5. Matched providers see it in their existing "Requests" queue (the same `request_matches` rows a normal search generates, filtered to `status = pending`).
6. Providers respond (interested/declined), same as a standard match.
7. User reviews interested providers using the same results-list component used elsewhere in the app.
8. User connects with a selected provider.

**Implementation note:** this reuses the NEED → MATCH pipeline end to end. There is no dedicated broadcast, bidding, or multi-response comparison system — that would be new infrastructure this scope decision deliberately avoids. The only new work is: (a) a "publish and wait" presentation instead of an immediate results screen, and (b) a provider-facing "Nearby Open Needs" list view over data that already exists. See Implementation Plan Phase 8A.
...
| FR-15 | Post a Need (reuses NEED→MATCH pipeline, §22) | Should |
...
2. ~~What exact formula should generate the Trust Score?~~ **Resolved** — see TRD §9 / Backend Schema §15, Canonical Trust Score Formula.
3. ~~Which trust signals should have the highest weight?~~ **Resolved** — same reference as above (Rating 20%, Response Rate 15%, Completed Jobs 15% are the three highest).
4. How should provider availability be represented?
5. ~~What minimum evidence should be required before recommending a provider?~~ **Resolved** — a completed connection is required; see §21 and Backend Schema §16.
6. ~~How should duplicate/fake community recommendations be handled?~~ **Resolved** — unique constraint per recommender/provider pair, see §21.
...
Remaining open items belong in subsequent technical and design documents rather than being prematurely locked in the PRD.
